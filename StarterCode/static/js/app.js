...
function init() {
    ...
    d3.json("../../samples.json").then((sample) => {
        ...
        console.log(sample)
        var sampleNames = sample.names[0];
        ...
        var metaData=sample.metadata[0];
        ...
        d3.json("../../samples.json").then((sample)=>{
            ...
            var otu_ids=sample.samples[0].otu_ids.slice(0,10).reverse();
            var sample_values=sample.samples[0].sample_values.slice(0,10).reverse();
            var otu_labels=sample.samples[0].otu_labels.slice(0,10).reverse();
            ...
            var bubbleIDs=sample.samples[0].otu_ids;
            var bubbleValues=sample.samples[0].sample_values;
            var bubbleLabels=sample.samples[0].otu_labels;
 ...
 function optionChanged() {
   ...
   d3.json("../../samples.json").then((sample)=>{
    ...
    //update metadata
    var metaData=sample.metadata[sampleID];
    d3.select("#sample-metadata")
    .selectAll("p")
    .remove();
    d3.select("#sample-metadata")
    ...
    var otu_ids=sample.samples[sampleID].otu_ids.slice(0,10).reverse();
    var sample_values=sample.samples[sampleID].sample_values.slice(0,10).reverse();
    var otu_labels=sample.samples[sampleID].otu_labels.slice(0,10).reverse();
    var upstrOtuIds=otu_ids.map(function(e){return "OTU "+e.toString()});
    var bubbleIDs=sample.samples[sampleID].otu_ids;
    var bubbleValues=sample.samples[sampleID].sample_values;
    var bubbleLabels=sample.samples[sampleID].otu_labels;
    marker={
        color: bubbleIDs,
        size: bubbleValues
        };     
